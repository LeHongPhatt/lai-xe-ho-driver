...
import { IconApp, RNFlatList, TextCus, TouchCus, ViewCus } from 'components';
...
const RestaurantDetail: React.FC = () => {
  ...
  }, [route.params?.restaurantId, listFoodCatalog, getListFoods]);
    ...
    <ViewCus f-1 bg-white>
      ...
      <Animated.ScrollView
        ...
        <FoodInfo
          ...
          rating={detailRestaurant?.average_rating ?? 0}
          review={detailRestaurant?.total_reviews ?? 0}
        />
        <ViewCus style={styles.content}>
          ...
          />
          <RNFlatList
            data={loading ? dataDefaults : listFoods}
      ...
      {/* <ViewCus
        ...
        <ViewCus flex-row items-center mr-24 style={styles.cart}>
          ...
          <ViewCus style={styles.circle}>
            <TextCus bold>{totalCart.length}</TextCus>
          </ViewCus>
        </ViewCus>
        <Buttons
          textBtn="Thanh toán"
          style={styles.btnAddFood}
          onPress={() => NavigationService.navigate(Routes.CartOrder)}
        />
      </ViewCus> */}
    </ViewCus>
  );
