...
import { IRefOTP } from './components/InputOtp';
...
const OTP: React.FC = () => {
  ...
  const refInputOtp = useRef<IRefOTP>(null);
  ...
  useEffect(() => {
    if (!isFocused || !route?.params?.isOTPExpired) {
      return;
    }
    resetForm();
    refInputOtp.current?.resetOtp?.();
    onPressResend();
  }, [isFocused, route?.params?.isOTPExpired]);
